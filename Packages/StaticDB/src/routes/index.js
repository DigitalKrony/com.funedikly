...
router.get('/:dir*?', function (req, res, next) {
    ...
    pageView === '/' ? pageView = 'index' : pageView = pageView.replace(new RegExp('/', 'gi'), '/');
    let pageViewParts = pageView.split('/');

    if (fs.existsSync(path.resolve(`./src/${pageView}/index.ejs`))) {
        let pageData = JSON.parse(fs.readFileSync(path.resolve(`./src/${pageView}/data/${pageViewParts[pageViewParts.length-1]}.json`)));

        let config = {
            theme: qsp.css.theme || 'df',
        ...
        let pageData = JSON.parse(fs.readFileSync(path.resolve(`./src/pages/data/${pageView}.json`)));
